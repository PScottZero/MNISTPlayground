...
export class NeuralNetworkService {
  ...
  epochCount: number;
  activationDerivSum: math.Number[];
        ...
        for (let epoch = 0; epoch < this.epochCount; epoch++) {
          ...
          for (let i = 0; i < trainX.length; i++) {
            if ((i + 1) % this.miniBatchSize === 0 || i === trainX.length - 1) {
              const accuracy = math.round(math.multiply(math.divide(this.totalCorrect, this.total), 100), 2);
  ...
  updateWeights(): void {
    ...
    console.log(avgGrad);
    const eta = math.multiply(1000, math.subtract(1, math.divide(this.totalCorrect, this.totalCorrect)));
    for (let i = 1; i < this.layers.length; i++) {
      for (let j = 0; j < this.layers[i].length; j++) {
        this.layers[i][j].weights = math.subtract(this.layers[i][j].weights,
          math.multiply(eta, avgGrad[i - 1][j].weightDeriv));
        this.layers[i][j].bias = math.subtract(this.layers[i][j].bias,
          math.multiply(eta, avgGrad[i - 1][j].weightDeriv));
      }
    }
  }
  ...
  calculateGradient(): NeuronDeriv[][] {
    ...
    for (let layerNo = this.layers.length - 1; layerNo > 0; layerNo--) {
      for (let neuronNo = 0; neuronNo < this.layers[layerNo].length; neuronNo++) {
        ...
        const neuronDeriv = gradient[layerNo - 1][neuronNo];
        neuronDeriv.biasDeriv = this.getCostBiasDeriv(layerNo, neuronNo);
        for (let weightNo = 0; weightNo < neuron.weights.length; weightNo++) {
          const biasDeriv = neuronDeriv.biasDeriv;
          neuronDeriv.weightDeriv.push(this.getCostWeightDeriv(layerNo, neuronNo, weightNo, biasDeriv));
          neuronDeriv.activationDeriv.push(this.getCostActivationDeriv(layerNo, neuronNo, weightNo, biasDeriv));
        }
      }

      this.activationDerivSum = Array<math.Number>(this.layers[layerNo - 1].length).fill(0);
      for (const neuronDeriv of gradient[layerNo - 1]) {
        for (let i = 0; i < neuronDeriv.activationDeriv.length; i++) {
          this.activationDerivSum[i] += neuronDeriv.activationDeriv[i];
        }
      }
    }
    return gradient;
  ...
  getCostBiasDeriv(layer: number, currNeuron: number): math.Number {
    ...
    return math.multiply(neuron.activation(neuron.value, true),
  ...
  getPrevCostActivationDeriv(layer: number, currNeuron): math.Number {
    if (layer === this.layers.length - 1) {
      ...
      return this.activationDerivSum[currNeuron];
    }
  }
}
